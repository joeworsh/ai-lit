...
class GbTitlesFlatLstmUniversity(gb_title_university.GbTitlesUniversity):
    """
    This is an AI Lit university for training LSTM on the Gutenberg titles text dataset.
    """
    ...
    def get_model(self, graph):
        """
        Build the LSTM model for the GB titles text exercise
        ...
        return flat_lstm.FlatLstm(len(self.vocab), len(self.subjects), 0, pretrained)

    def perform_training_run(self, session, model, batch_y, batch_x):
        """
        Perform a training run of the model on the GB titles batch.
    ...
    def perform_evaluation_run(self, session, model, batch_y, batch_x):
        """
        Perform a validation or evaluation run of the model on the GB titles text batch.
