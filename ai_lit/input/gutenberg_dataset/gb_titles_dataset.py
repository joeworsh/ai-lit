...
titles of each book as a sequence of indices and the associated atomic genre which
...
TRAIN_TITLE_RECORDS = 'train_gb_titles.tfrecords'
...
def compile_dataset(subjects, dataset_dir, workspace, max_vocab_size=5000, test_split=0.3):
    """
    Assemble all needed components in order to build the GB titles dataset.
...
def write_record(gidx, vocab, subjects, tf_writer):
    ...
    """
    terms = [input_util.lookup_word(term, vocab) for term in nltk.word_tokenize(gidx.title)]
